...
class YarnSparkHadoopUtil extends SparkHadoopUtil {
   ...
   */
  private[yarn] def obtainTokenForHiveMetastoreInner(conf: Configuration): Option[Token[DelegationTokenIdentifier]] = {
    ...
    if (metastoreUri.nonEmpty) {
      ...
      logDebug(s"Getting Hive delegation token for ${currentUser.getUserName()} against " + s"$principal at $metastoreUri")
      ...
      try {
        ...
        doAsRealUser {
          ...
          val tokenStr = getDelegationToken.invoke(hive, currentUser.getUserName(), principal).asInstanceOf[String]
